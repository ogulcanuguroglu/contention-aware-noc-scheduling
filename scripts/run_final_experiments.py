...
def validate_dataframe(df: pd.DataFrame) -> None:
    ...
    assert (heft["replayed_speedup_vs_heft"] == 1.0).all(), "HEFT replayed_speedup_vs_heft must be 1.0"
...
- **No recursive ancestor duplication**: The proposed scheduler duplicates only
  ...
  a later phase) could further reduce makespan.
...
- **runtime_ms**: Measures scheduler execution time only, recorded by
  `time.perf_counter()` around `scheduler.schedule(dag)`. Fair replay
  evaluation via `replay_under_contention()` runs after scheduling completes
  and is not included in `runtime_ms`.
"""
    ...
    lines = [
        ...
        "> **Phase 13 note**: Fair contention replay metrics are included in all"
        " CSVs generated by this script: `replayed_makespan`,"
        " `replayed_speedup_vs_heft`, `replayed_communication_count`,"
        " `replayed_max_link_utilization`, `replay_overhead_ratio`,"
        " `replayed_vs_original_delta`. These columns replay every scheduler's"
        " placement decisions under the common contention-aware model for fair"
        " physical comparison. CSV files generated after Phase 13 include these"
        " columns; this script now writes them when generating the CSV.\n",
        config_section,
        mean_section,
        best_section,
        ccr_section,
